function [Cost,Pure_Cost,Pressure_Nodes,Length_Pipes] = DNYTP_Cost2(Pipe)
global  G_NYTP
if size(Pipe,1)==1
   Pipe=Pipe'; 
end
...
if Sum_Violation ~=0
    ...
    %end
Cost(k) = Sum_Pipe_Cost + Penalty_Pressure; %+ (Sum_dif * 1000000); % penalty factor=12,000,000
else
Cost(k) = Sum_Pipe_Cost;% + (Sum_dif * 1000000)  ;
end
Pure_Cost(k) = Sum_Pipe_Cost;
end % end for
